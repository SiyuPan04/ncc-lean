...
/-!
# Instantiating the concrete upper system from the NC--C class

This file discharges the analytic existence obligations of
`OuterTrajectoryConcrete.OuterSystem` from the fields of `IsNCCClass`.
No saddle point, projection, conjugate-finiteness, or micro-oracle premise is
added to the class.
-/

namespace NCCLowerBoundVerification
namespace Upper
namespace SystemInstantiation
...
private theorem section_lower_bound {m n : Nat} {ell D Delta : ℝ}
    ...
    lower -
      ...
      P.f x y := by
  obtain ⟨ym, hym⟩ := hP.maximum_attained x hx
...
theorem gammaBddAbove_of_class {m n : Nat} {ell D Delta r : ℝ}
  ...
  obtain ⟨lower, hlower⟩ := IsNCCClass.exists_value_lower_bound hP
...
private def affineLine {d : Nat} (x v : EVec d) (t : ℝ) : EVec d :=
  t • v + x

private theorem affineLine_zero {d : Nat} (x v : EVec d) :
    affineLine x v 0 = x := by
  simp [affineLine]

private theorem affineLine_one_sub {d : Nat} (x u : EVec d) :
    affineLine x (u - x) 1 = u := by
  ext i
  simp [affineLine]

private theorem affineLine_mem_convex {d : Nat} {C : Set (EVec d)}
    (hC : Convex ℝ C) {x u : EVec d} (hx : x ∈ C) (hu : u ∈ C)
    {t : ℝ} (ht : t ∈ Set.Icc (0 : ℝ) 1) :
    affineLine x (u - x) t ∈ C := by
  have h := hC hx hu (sub_nonneg.mpr ht.2) ht.1 (by linarith)
  convert h using 1
  ext i
  simp only [affineLine, Pi.add_apply, Pi.sub_apply, Pi.smul_apply,
    smul_eq_mul]
  ring

private theorem hasDerivAt_primal_line {m n : Nat}
    {ell D Delta : ℝ} {P : NCCInstance m n}
    (hP : IsNCCClass ell D Delta P)
    (x v : EVec m) (y : EVec n) (t : ℝ) :
    HasDerivAt (fun s : ℝ => P.f (affineLine x v s) y)
      (dot (P.gradX (affineLine x v t) y) v) t := by
  let L : ℝ →L[ℝ] EVec m := (1 : ℝ →L[ℝ] ℝ).smulRight v
  have hxline : HasFDerivAt (affineLine x v) L t := by
    simpa [affineLine, L, ContinuousLinearMap.smulRight_apply] using
  ...
  have houter : HasFDerivAt (Function.uncurry P.f)
      (fderiv ℝ (Function.uncurry P.f) (affineLine x v t, y))
      (affineLine x v t, y) :=
    hP.gradient_representation.1.differentiableAt.hasFDerivAt
  have hderiv := (houter.comp t hpath).hasDerivAt
  apply hderiv.congr_deriv
  rw [ContinuousLinearMap.comp_apply]
  have hL : (L.prod (0 : ℝ →L[ℝ] EVec n)) 1 = (v, 0) := by
    ext i <;> simp [L, ContinuousLinearMap.smulRight_apply]
  rw [hL, hP.gradient_representation.2]
  simp [dot]

private theorem hasDerivAt_dual_line {m n : Nat}
    {ell D Delta : ℝ} {P : NCCInstance m n}
    (hP : IsNCCClass ell D Delta P)
    (x : EVec m) (y v : EVec n) (t : ℝ) :
    HasDerivAt (fun s : ℝ => P.f x (affineLine y v s))
      (dot (P.gradY x (affineLine y v t)) v) t := by
  let L : ℝ →L[ℝ] EVec n := (1 : ℝ →L[ℝ] ℝ).smulRight v
  have hxconst : HasFDerivAt (fun _ : ℝ => x) 0 t :=
    hasFDerivAt_const (𝕜 := ℝ) x t
  have hyline : HasFDerivAt (affineLine y v) L t := by
    simpa [affineLine, L, ContinuousLinearMap.smulRight_apply] using
      L.hasFDerivAt.add_const y
  have hpath := hxconst.prodMk hyline
  have houter : HasFDerivAt (Function.uncurry P.f)
      (fderiv ℝ (Function.uncurry P.f) (x, affineLine y v t))
      (x, affineLine y v t) :=
    hP.gradient_representation.1.differentiableAt.hasFDerivAt
  have hderiv := (houter.comp t hpath).hasDerivAt
  apply hderiv.congr_deriv
  rw [ContinuousLinearMap.comp_apply]
  have hL : ((0 : ℝ →L[ℝ] EVec m).prod L) 1 = (0, v) := by
    ext i <;> simp [L, ContinuousLinearMap.smulRight_apply]
  rw [hL, hP.gradient_representation.2]
  simp [dot]

private theorem hasDerivAt_vecSq_line {d : Nat}
    (x v : EVec d) (t : ℝ) :
    HasDerivAt (fun s : ℝ => vecSq (affineLine x v s))
      (2 * dot (affineLine x v t) v) t := by
  unfold vecSq NCPLVerification.vecSq
  have hsum : HasDerivAt
      (fun s : ℝ => ∑ i : Fin d, (affineLine x v s i) ^ 2)
      (∑ i : Fin d, 2 * affineLine x v t i * v i) t := by
    apply HasDerivAt.fun_sum
    intro i _
    have hlin : HasDerivAt (fun s : ℝ => s * v i + x i) (v i) t := by
      convert ((hasDerivAt_id t).mul_const (v i)).add_const (x i) using 1
...
private theorem derivative_nonneg_of_minOn_Icc {g : ℝ → ℝ} {g' : ℝ}
    (hmin : IsMinOn g (Set.Icc (0 : ℝ) 1) 0)
    (hderiv : HasDerivAt g g' 0) : 0 ≤ g' := by
  have htangent : (1 : ℝ) ∈ posTangentConeAt (Set.Icc (0 : ℝ) 1) 0 := by
    have h := sub_mem_posTangentConeAt_of_segment_subset
      ...
      (by simp [segment_eq_Icc (by norm_num : (0 : ℝ) ≤ 1)])
    simpa using h
  have h := hmin.localize.hasFDerivWithinAt_nonneg
    hderiv.hasFDerivAt.hasFDerivWithinAt htangent
  simpa using h

private theorem derivative_nonpos_of_maxOn_Icc {g : ℝ → ℝ} {g' : ℝ}
    (hmax : IsMaxOn g (Set.Icc (0 : ℝ) 1) 0)
    (hderiv : HasDerivAt g g' 0) : g' ≤ 0 := by
  have htangent : (1 : ℝ) ∈ posTangentConeAt (Set.Icc (0 : ℝ) 1) 0 := by
    have h := sub_mem_posTangentConeAt_of_segment_subset
      (s := Set.Icc (0 : ℝ) 1) (x := (0 : ℝ)) (y := (1 : ℝ))
      (by simp [segment_eq_Icc (by norm_num : (0 : ℝ) ≤ 1)])
...
theorem saddleOperator_stronglyMonotoneOn {m n : Nat}
    ...
    (hP : IsNCCClass ell D Delta P) (hr : 0 < r)
  ...
  have hmon := ClassOperator.saddleGradientMonotoneOn (r := r) hP z
...
theorem saddleOperator_lipschitzOn {m n : Nat}
  ...
  let sx := gx + ell • dx
  let sy := -gy + r • dy
  have hsource := ClassOperator.jointGradientLipschitzOn hP z
    (unscaleX ell u) hu.1 (unscaleY ell u) hu.2
    (unscaleX ell v) hv.1 (unscaleY ell v) hv.2
...
theorem exists_saddleWitness {m n : Nat} {ell D Delta r : ℝ}
  ...
  obtain ⟨u, hu, hnormal⟩ :=
    ProjectedVIExistence.exists_projectedVI_solution
      (ClassOperator.scaledSet_nonempty hP)
      (ClassOperator.scaledSet_closed hP)
      (ClassOperator.scaledSet_convex hP)
...
theorem SaddleWitness.primal_minimizes {m n : Nat}
  ...
  intro u hu
  let d := u - W.x
  have hsupp := ClassOperator.convexSupportX (r := r) hP z
...
theorem SaddleWitness.dual_maximizes {m n : Nat}
  ...
  intro v hv
  let d := v - W.y
  have hsupp := ClassOperator.concaveSupportY (r := r) hP z
...
theorem SaddleWitness.isProxPoint {m n : Nat}
  ...
  obtain ⟨yu, hyu⟩ := IsNCCClass.regularized_maximum_attained hP u
...
def stationaryAt_of_class {m n : Nat} {ell D Delta r : ℝ}
  ...
  have hxEq : selectedProx hexistsR z = W.x :=
    selectedProx_unique hP.ell_pos
      (IsNCCClass.regularizedValue_weaklyConvex hP) hexistsR hprox
  ...
  have hsub : IsGammaSubgradient P.X P.Y (decurved P.f ell r z)
    ...
    apply gamma_subgradient (gammaBddAbove_of_class hP z)
      ...
      (ClassOperator.convexSupportX (r := r) hP z
        (selectedProx_spec hexistsR z).1 W.y_mem)
      (ClassOperator.concaveSupportY (r := r) hP z
        (selectedProx_spec hexistsR z).1 W.y_mem)
      hnormalX hnormalY
  refine {
    qStar := qStar
...
theorem regularizedValue_hasProxEverywhere_of_class {m n : Nat}
  ...
  obtain ⟨lower, hlower⟩ := IsNCCClass.exists_value_lower_bound hP
  obtain ⟨K, hK0, hK⟩ := regularized_dual_sq_bound hP
  have hlowerR : ∀ x ∈ P.X,
      lower - r / 2 * K ≤ DualRegularizedValueOn P.Y P.f r x := by
    intro x hx
    obtain ⟨y, hy⟩ := hP.maximum_attained x hx
    obtain ⟨yr, hyr⟩ := IsNCCClass.regularized_maximum_attained hP x
    have hv := hyr.2 y hy.1
    have hvalue := value_eq_of_isMaximizerOn hy
    have hvalueR := value_eq_of_isMaximizerOn hyr
    have hl := hlower x hx
    rw [hvalue] at hl
    change P.f x y - r / 2 * vecSq y ≤
      P.f x yr - r / 2 * vecSq yr at hv
    unfold DualRegularizedValueOn
    rw [hvalueR]
    have hyK := hK y hy.1
    nlinarith
  exact hasProxEverywhere_of_continuousOn_bddBelow
    ...
    (IsNCCClass.regularizedValue_continuous hP).continuousOn hlowerR
...
theorem classOracle_certificate {m n : Nat} {ell D Delta r : ℝ}
  ...
  have cert := feasibleMicro_scaledOperator_of_closedConvex
    ...
    hprojX hprojY S
    (ClassOperator.gradXHat P ell z) (ClassOperator.gradYHat P ell z)
    (ClassOperator.saddleGradientMonotoneOn (r := r) hP z)
    (ClassOperator.jointGradientLipschitzOn hP z)
  dsimp only at cert
...
theorem classOracle_subgradient {m n : Nat} {ell D Delta r : ℝ}
  ...
  exact gamma_subgradient (gammaBddAbove_of_class hP z)
    cert.1
    (ClassOperator.convexSupportX (r := r) hP z cert.1 cert.2.1)
    (ClassOperator.concaveSupportY (r := r) hP z cert.1 cert.2.1)
