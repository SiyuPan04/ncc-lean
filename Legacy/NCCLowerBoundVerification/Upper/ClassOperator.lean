...
/-!
# The actual decurved class operator

This file derives the support, saddle-monotonicity, Lipschitz, and scaled-set
facts needed by the feasible micro solver directly from `IsNCCClass`.
-/
...
open PointwiseConjugate ProjectionGeometry ScaledOperator

def gradXHat {m n : Nat} (P : NCCInstance m n) (ell : ℝ)
    (z x : EVec m) (y : EVec n) : EVec m :=
  P.gradX x y + ell • x - (2 * ell) • z

def gradYHat {m n : Nat} (P : NCCInstance m n)
    (_ell : ℝ) (_z : EVec m) (x : EVec m) (y : EVec n) : EVec n :=
  P.gradY x y
