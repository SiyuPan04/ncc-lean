import NCC.Extensions.NCSCGeometry
import NCCLowerBoundVerification.Upper.ClassOperator

/-! Generic solver operator proofs with no compactness or dual diameter.
The proofs follow the checked within-domain operator algebra, now with the
explicit unbounded-domain CoreClass premises instantiated by NCSCGeometry. -/

namespace NCC.Extensions.NCSCOperator
noncomputable section
open NCC.Model NCC.Extensions.NCSCGeometry NCCLowerBoundVerification NCCLowerBoundVerification.Upper
open PointwiseConjugate ProjectionGeometry ScaledOperator ClassOperator

theorem convexSupportX {m n : Nat} {ell r : ℝ}
...
theorem concaveSupportY {m n : Nat} {ell r : ℝ}
    {P : NCCInstance m n} (hP : CoreClass ell r P)
    (z : EVec m) {x : EVec m} (hx : x ∈ P.X)
    {y : EVec n} (hy : y ∈ P.Y) :
...
theorem saddleGradientMonotoneOn {m n : Nat} {ell r : ℝ}
    {P : NCCInstance m n} (hP : CoreClass ell r P)
    (z : EVec m) :
...
theorem jointGradientLipschitzOn {m n : Nat} {ell r : ℝ}
    {P : NCCInstance m n} (hP : CoreClass ell r P)
    (z : EVec m) :
...
theorem scaledSet_nonempty {m n : Nat} {ell r : ℝ}
    {P : NCCInstance m n} (hP : CoreClass ell r P) :
...
theorem scaledSet_closed {m n : Nat} {ell r : ℝ}
    {P : NCCInstance m n} (hP : CoreClass ell r P) :
...
theorem scaledSet_convex {m n : Nat} {ell r : ℝ}
    {P : NCCInstance m n} (hP : CoreClass ell r P) :
...
end NCC.Extensions.NCSCOperator
